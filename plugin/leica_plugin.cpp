...
namespace gazebo
{
  ...
  ////////////////////////////////////////////////////////////////////////////////
  // Load the controller
  void GazeboRosLaser::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf) //cambiar GazeboRosLaser por LeicaC5Plugin o algo así
  {
    // load plugin
    ...
    ros::SubscribeOptions so =
        ros::SubscribeOptions::create<sensor_msgs::RegionOfInterest>("/" + this->parent_ray_sensor_->Name() + "/laser_window_config", 1, boost::bind(&GazeboRosLaser::ScanWindowConfig, this, _1), ros::VoidPtr(), &this->rosQueue);
    this->rosSub = this->rosNode->subscribe(so);

    // Spin up the queue helper thread.
    this->rosQueueThread = std::thread(boost::bind(&GazeboRosLaser::QueueThread, this));
  }
  ...
  void GazeboRosLaser::LoadThread()
  {
    ...
    ROS_INFO_NAMED("gpu_laser", "GPU Laser Plugin (ns = %s) <tf_prefix_>, set to \"%s\"", this->robot_namespace_.c_str(), this->tf_prefix_.c_str());
  ...
  void GazeboRosLaser::LaserConnect()
  {
    ...
    cv.wait(lk, [&]{return start_laser;});
    if (this->laser_connect_count_ == 1)
      this->laser_scan_sub_ = this->gazebo_node_->Subscribe(this->parent_ray_sensor_->Topic(), &GazeboRosLaser::OnScan_gzSensorMsg, this);
    lk.unlock();
  }
  ...
  void GazeboRosLaser::OnScan_gzSensorMsg(ConstLaserScanStampedPtr &_msg) //gzSensorMsg_to_ros_pub
  {
  ...
  void GazeboRosLaser::ScanWindowConfig(const sensor_msgs::RegionOfInterestConstPtr &_msg)
  {
    // UNDER CONSTRUCTION...
    // parent_ray_sensor_->SetAngleMax(atan(tan(_msg->y_offset) + (_msg->height) / 2));
    // parent_ray_sensor_->SetAngleMin(atan(tan(_msg->y_offset) - (_msg->height) / 2));
    std::lock_guard<std::mutex> lk(m);
    start_laser = true;
    cv.notify_one();
  }

  void GazeboRosLaser::QueueThread()
  {
